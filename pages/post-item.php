    ...
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($category) || empty($item_type) || empty($title) || empty($description) || empty($location) || empty($date_lost_found)) {
            ...
            if ($result['success']) {
                ...
                echo '<div class="fixed top-4 right-4 bg-green-500 text-white p-4 rounded-lg shadow-lg z-50">' . $success . '</div>';
...
<div class="container mx-auto px-4 py-12 max-w-2xl">
    <div class="bg-white rounded-lg shadow-lg p-8">
        <h1 class="text-3xl font-bold mb-2">Post an Item</h1>
        ...
        <form method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="item_type" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-600">
                ...
                <div>
                    ...
                    <select name="category" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-600">
            ...
            <div>
                ...
                <textarea name="description" required placeholder="Provide detailed information about the item..."
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-600 h-32"><?php echo escape($_POST['description'] ?? ''); ?></textarea>
            </div>
            ...
            <div>
                ...
                <textarea name="distinguishing_features" placeholder="Any unique marks, scratches, serial numbers, etc."
                          class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-600 h-24"><?php echo escape($_POST['distinguishing_features'] ?? ''); ?></textarea>
            </div>

            <!-- Image Upload -->
            <div>
                <label class="block text-sm font-semibold mb-2">Item Image</label>
                <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center cursor-pointer hover:border-blue-600 transition"
            ...
            <button type="submit" class="w-full bg-blue-600 text-white font-bold py-3 rounded-lg hover:bg-blue-700 transition">
                <i class="fas fa-check-circle"></i> Post Item
            </button>
        </form>
    </div>
</div>
